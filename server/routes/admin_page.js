...
router.get("/listFhirMessage/:id", async (req, res) => {
  const fhirResponse = await CompositionController.listFhirMessageByID(req.params.id);
  res.status(200).json(fhirResponse.response)
});
...
function fhirToComposition(fhirMessage) {
  let composition = {
    ...
    "items.0.6.items.2.value.date": "entry.9.entry.effective.effectiveDateTime.date",
    "items.0.6.items.2.value.time": "entry.9.entry.effective.effectiveDateTime.time",
